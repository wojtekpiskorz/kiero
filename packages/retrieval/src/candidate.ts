/**
 * The pinned initial index candidate: which embedding model, dimensions,
 * text preparation and provider route version one index generation must be
 * built from.
 *
 * The bounded solution: "Use the Qwen candidate at the initial
 * 4096 dimensions only after the live route proves it." The pins mirror
 * @kiero/providers' routing constants (provider routing owns them); this package pins its
...
/** The version label of the frozen embedding route this candidate rides. */
export const PROVIDER_ROUTE_VERSION = "e2.embedding.route.v1";

/** The pinned embedding model of the initial candidate (the first choice). */
export const INDEX_EMBEDDING_MODEL = "qwen/qwen3-embedding-8b";

/** The pinned native dimensions of the initial candidate (baseline). */
