...
import { HEARTBEAT_SERVICES, HEARTBEATS_KEPT_PER_SERVICE } from "../../convex/operations/telemetry/heartbeat";
...
import { classifyIncidents, STUCK_RUN_MS } from "../../convex/operations/telemetry/incidents";

const fixturesDir = path.dirname(fileURLToPath(import.meta.url));
const observabilityDir = path.resolve(fixturesDir, "../../infra/observability");

function readJson(name: string): Record<string, unknown> {
  return JSON.parse(readFileSync(path.join(observabilityDir, name), "utf8")) as Record<string, unknown>;
}
...
function literalMembers(
  ...
  const validator = schema.tables[table]?.validator as unknown as LooseValidator | undefined;
  if (validator === undefined || validator.kind !== "object" || validator.fields === undefined) {
    throw new Error(`${table}: expected an object validator`);
  }
  const field = validator.fields[column] as LooseField | undefined;
  if (field === undefined || field.kind !== "union" || field.members === undefined) {
...
describe("telemetry fragment tables", () => {
  it("adds exactly the three I2 tables to the closed inventory", () => {
    ...
    // 70 after G3's calendarSyncAttempts/calendarProofEvents; 72 after F3's
    // pushDeliveries and proof-only pushProofDevices.
    expect(TABLE_ID_NAMES).toHaveLength(72);
  });
  ...
  it("the table unions are pinned to the single model definitions", () => {
    ...
    expect(new Set(literalMembers("costEntries", "provider"))).toEqual(new Set(SPEND_PROVIDERS));
  });

  it("carries the indexes the monitors and retention need", () => {
    type IndexableTable = { " indexes"(): { indexDescriptor: string; fields: string[] }[] };
    const indexFields = (
      table: "diagnosticEvents" | "healthHeartbeats" | "costEntries" | "costAlertStates",
      index: string,
    ): string[] => {
      const tableDefinition = schema.tables[table] as IndexableTable | undefined;
      if (tableDefinition === undefined) {
        throw new Error(`${table}.${index} missing`);
      }
      const found = tableDefinition[" indexes"]().find((d) => d.indexDescriptor === index);
      if (found === undefined) {
        throw new Error(`${table}.${index} missing`);
      }
      return [...found.fields];
    ...
    expect(indexFields("diagnosticEvents", "by_kind_time")).toEqual(["kind", "atMs"]);
    expect(indexFields("healthHeartbeats", "by_service_time")).toEqual(["serviceName", "atMs"]);
    expect(indexFields("costEntries", "by_period_provider")).toEqual(["period", "provider"]);
    expect(indexFields("costAlertStates", "by_period_level")).toEqual(["period", "level"]);
  });
});
...
describe("descriptors cannot drift from the single definitions", () => {
  ...
  it("retention.json matches the windowing constants", () => {
    ...
    expect(descriptor.healthHeartbeats.keptPerService).toBe(HEARTBEATS_KEPT_PER_SERVICE);
  });

  it("the honesty block states the Free-plan blind spots", () => {
    expect(OBSERVABILITY_HONESTY.nativeConvexLogHistory).toBe("unavailable_on_free_plan");
    expect(OBSERVABILITY_HONESTY.applicationEvents).toBe("explicit_redacted_events_only");
    expect(OBSERVABILITY_HONESTY.diagnosticWindowDays).toBe(30);
    expect(OBSERVABILITY_HONESTY.blindSpots.length).toBe(3);
    expect(OBSERVABILITY_HONESTY.authority).toBe("domain_tables_canonical_telemetry_best_effort");
  });
});
...
describe("incident classification (monitor scan 1)", () => {
  ...
  it("diagnoses attempts-exhausted durable jobs exactly once (dedup by jobKey)", () => {
    const incidents = classifyIncidents(
      {
        jobs: [
          { jobKey: "job_abc123-def456", kind: "processing.transcribe_segment", state: "failed", attempts: 3, maxAttempts: 3, lastErrorKind: "echo_target_not_configured" },
          { jobKey: "job_retry-pending", kind: "platform.echo_delivery", state: "failed", attempts: 1, maxAttempts: 3 }, // not exhausted
          { jobKey: "job_ok-000000", kind: "platform.echo_delivery", state: "succeeded", attempts: 1, maxAttempts: 3 },
        ],
        outbox: [],
        runs: [],
      },
    ...
    const sanitized =
      incident === undefined
        ...
        : sanitizeDiagnosticEvent({ kind: incident.kind, metadata: incident.metadata });
    ...
    const incidents = classifyIncidents(
      {
        jobs: [],
        outbox: [
          { eventId: "c4d62f5c-0000-4000-8000-000000000000", eventName: "sources.sourceAccepted", deliveryState: "failed", attempts: 1, lastErrorKind: "consumer_projection_missing" },
          { eventId: "c4d62f5c-0000-4000-8000-000000000001", eventName: "platform.echoRequested", deliveryState: "delivered", attempts: 1 },
        ],
        runs: [
          { runId: "p97bxdbr9x2jd9x73ha8dtc9hkchm0f1", state: "running", startedAtMs: nowMs - STUCK_RUN_MS - 1 },
          { runId: "p97bxdbr9x2jd9x73ha8dtc9hkchm0f2", state: "running", startedAtMs: nowMs - 60_000 },
          { runId: "p97bxdbr9x2jd9x73ha8dtc9hkchm0f3", state: "succeeded", startedAtMs: nowMs - STUCK_RUN_MS - 1 },
        ],
      },
      nowMs,
    );
