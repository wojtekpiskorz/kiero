...
describe("the shipped host features", () => {
  ...
  it("registers every feature as pending with a note, except the mounted lanes (H1 conversation + memory, H3 search + source detail, B3 membership, B4 GM, G1 calendar, F3 notifications, H4 GM processing, D4 capture); only the conversation carries its own placeholder screen", () => {
    ...
    // mounts the evidence-search and source-history routes.
