...
describe("the shipped host features", () => {
  ...
  it("registers every feature as pending with a note, except the mounted lanes (H1 conversation + memory, H3 search + source detail, B3 membership, B4 GM, G1 calendar, F3 notifications); only the conversation carries its own placeholder screen", () => {
    ...
    // screen; H3 (issue #51) mounts the evidence-search and source-history
    // routes.
