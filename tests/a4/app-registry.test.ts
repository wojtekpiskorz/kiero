...
describe("the shipped host features", () => {
  ...
  it("registers every feature as pending with a note, except the mounted lanes (B3 membership, B4 GM, G1 calendar); only the conversation carries its own placeholder screen", () => {
    ...
    // G1 (issue #45) mounts the Calendar connection screen.
    const mounted = appFeatures.filter((entry) => entry.implementation === "mounted");
    expect(mounted.map((entry) => entry.featureId)).toEqual(["access.membership", "access.gm", "calendar.connection"]);
