...
describe("the manifest seam (R34: Convex mutations cannot fetch)", () => {
  const source = readFileSync(new URL("../../convex/processing/audio/executor.ts", import.meta.url), "utf8");

  it("the workflow resolves the duration through an ACTION step before the mutation commits", () => {
    expect(source).toContain(
      "step.runAction(\n      internal.processing.audio.executor.resolveManifestInput",
    );
    expect(source).toContain(
      "step.runMutation(internal.processing.audio.executor.ensureManifest",
    );
  });
