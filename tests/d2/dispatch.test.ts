...
beforeEach(async () => {
  ...
  actor = await seedAuthedActor(ctx, "d2dispatch");
  authenticateAs(ctx, actor);
});
...
describe("the certified client operations", () => {
  ...
  it("dispatches prepareUpload through the bridge identity path", async () => {
    ...
    const unknown = await dispatchUploadsCommand(asTx(ctx), command("sources.unknownOp", {}));
  ...
  it("denies a revoked registry session through the canonical resolution", async () => {
  ...
  it("denies an upstream-signed-out session (registry row without authSessions)", async () => {
    const authSession = ctx.db.rows("authSessions").find((row) => row.userId === actor.userId);
    if (authSession !== undefined) {
      await ctx.db.patch("authSessions", authSession._id, { expirationTime: Date.now() - 1_000 });
    }
    const result = await dispatchUploadsCommand(
      asTx(ctx),
      command("sources.prepareUpload", { draftId: "dispatch-3b", parts: 2, mediaKinds: ["audio"] }),
    );
    expect(result._tag).toBe("error");
    expect(errorOf(result)._tag).toBe("unauthenticated");
  });
...
describe("the gateway protocol steps", () => {
  it("routes prepare through the certified operation (one implementation)", async () => {
    const result = await dispatchUploadsStep(asTx(ctx), {
      step: "prepare",
      input: { draftId: "step-1", parts: 2, mediaKinds: ["image"] },
    });
    expect(result._tag).toBe("ok");
    expect(valueOf(result)).toEqual({ uploadId: expect.any(String), stage: "draft" });
  });
  ...
  it("fails unknown steps and malformed envelopes typed", async () => {
    ...
    const unknown = await dispatchUploadsStep(asTx(ctx), { step: "rollback", input: {} });
    expect(unknown._tag).toBe("error");
    expect(errorOf(unknown)._tag).toBe("validation");

    const malformed = await dispatchUploadsStep(asTx(ctx), { step: "begin" });
  ...
  it("checks tenant scope on every step (cross-tenant upload denied)", async () => {
    const prepared = await dispatchUploadsStep(asTx(ctx), {
      step: "prepare",
      input: { draftId: "step-2", parts: 2, mediaKinds: ["image"] },
    });
    const uploadId = valueOf(prepared).uploadId as string;
    const other = await seedAuthedActor(ctx, "d2dispatch-b");
    authenticateAs(ctx, other);
    const denied = await dispatchUploadsStep(asTx(ctx), {
      step: "finalize",
      input: { uploadId },
    });
  ...
  it("checks identity BEFORE input decode (the certified order, no drift)", async () => {
    ...
    await ctx.db.patch("sessions", actor.sessionId, { revokedAtMs: Date.now() });
    const revoked = await dispatchUploadsStep(asTx(ctx), {
      step: "begin",
      input: { uploadId: 42 },
    });
    ...
    const fresh = await seedAuthedActor(ctx, "d2dispatch-order");
    authenticateAs(ctx, fresh);
    const malformed = await dispatchUploadsStep(asTx(ctx), {
      step: "begin",
      input: { uploadId: 42 },
    });
    expect(malformed._tag).toBe("error");
  ...
  it("sanitizes handler throws to unavailable (never a raw internal error)", async () => {
    ...
    const prepared = await dispatchUploadsStep(asTx(ctx), {
      step: "prepare",
      input: { draftId: "step-3", parts: 2, mediaKinds: ["image"] },
    });
    const uploadId = valueOf(prepared).uploadId as string;
