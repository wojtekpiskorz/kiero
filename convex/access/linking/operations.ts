/**
 * Access linking operations: the typed command registrations through
 * the runtime seam.
 *
 ...
 *   here per the product rule. GM access registered its contract entry and its only
...
/** The recovery command's pinned input (checked decode; GM access wires the invoker). */
...
/**
 * The checked manual-recovery command (defined; the GM access lane supplies the invoker).
 * `run` takes the RESOLVED GM ACTOR as
...
export const recoverAccountCommand = {
  ...
  /**
   * The handler runs the recovery core; only the GM dispatch (and the
...
/**
 * The handler table, composed into the access dispatch by the entry
