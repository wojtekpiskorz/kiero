 ...
 *
 * Notes (Convex Auth integration, candidate per Q186):
 *
 ...
 *   `createOrUpdateUser` policy (./userPolicy.ts); the library's
 ...
 *   mirrors; service-bridge sessions carry no `authSessionId`.
...
export const identityTables = {
   ...
   * (docs/research/auth-identity-facts.md); verified linking lives in ../linking.
   */
  users: defineTable({
    email: v.string(),
  ...
  /**
   * Live session device rows; revocation is immediate and audited.
