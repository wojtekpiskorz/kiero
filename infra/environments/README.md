...
1. No command in this repository may target staging or alpha production by
   default: the root `convex.json` names the dev project (staging is a
   non-default named deployment inside it, so bare commands resolve to the
   project's default dev/production deployments, never to `staging`), and
   every `apps/*/wrangler.jsonc` keeps its top-level (default) environment
   block pointed at `kiero-dev-*` names. Staging and production exist only as
   named environments (`--env staging`, `--env alpha-production`) with their
   own `kiero-staging-*` / `kiero-alpha-*` resource names.
