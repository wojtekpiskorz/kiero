...
| `ALLOWED_APP_ORIGINS` | worker var (`apps/gateway/src/index.ts`) | CORS origin allow-list; media routes authorize cross-origin calls only from the listed web origins (staging : the `https://kiero-staging-web.wojtek-524.workers.dev` static-assets Worker origin) |
| `IMAGES` | Cloudflare Images binding (optional) | preferred photo-normalization path when bound; bound on staging (transformations only, no storage; owner decision 2026-09-15) |
