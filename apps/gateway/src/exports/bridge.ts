/**
 * The Convex export-access channel bridge client: how the gateway's
 * download route consults the CURRENT authorization decision AS THE END
 * USER (the media bridge pattern, one transport: `postBridge`).
 *
 * The browser's Convex Auth credential is forwarded verbatim; Convex
 * resolves the acting person (live session -> membership -> company),
 * applies the CURRENT-administrator check and the lifecycle gate, and
