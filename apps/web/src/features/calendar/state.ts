...
export function commandFailureHint(error: { readonly _tag?: string; readonly code?: string }): string {
  // The closed contract puts the KIND in _tag (notFoundError answers
  // _tag "not_found", code "record_not_found"); the kind decides the copy,
  // because a vanished record is not a retryable hiccup.
  if (error._tag === "not_found") {
    return settingsCopy.copyNotFound;
  }
  if (error._tag === "unavailable") {
