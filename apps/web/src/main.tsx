...
// The push module joins the composition together with the service worker
// script that owns it.
import { webPushEntry } from "./pwa/push";
// The safe update module joins the same registration; its version source is the
...
/**
 * Bootstrap entry for the Kiero PWA host.
 *
 * Wires React 19 + TanStack Router + TanStack Query with the Convex
 * React-Query adapter exactly as the platform proof established, reads the typed config seam
 * once, mounts the application host and prepares the PWA entry
 * composition. The push module attaches the push-owning service worker (`/sw.js`: push
 * presentation + click-through only, no fetch/cache - protected data stays
 * behind live authorized queries); the update module attaches the safe update flow.
...
// The push module attaches the push-owning service worker through the composition (the
// only registration path); the push module's own hook stays passive (see
// apps/web/src/pwa/push.ts). The update module attaches to the
