...
4. **Vocabulary pins.** Every fragment-level closed union (job kinds/states, task/event/checklist states, project stages, upload stages, media roles, publication and export states, extension field kinds, membership roles, outbox delivery states) is annotated `ValueValidator<Encoded<typeof ContractsSchema>>` with the mechanism exported from `convex/schema/shared.ts`. Vocabulary ownership stays in the fragment; drift fails typecheck and the runtime equality test in `fragments.test.ts`.
