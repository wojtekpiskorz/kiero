...
| `convex/platform/schema.ts` | `processingRuns`, `processingSteps`, `processingAttempts`, `durableJobs` | A3 (composition), H4 (inspection), D6/E2 (stages) |
...
50 tables total; the inventory is the closed `TABLE_ID_NAMES` union in `@kiero/contracts`, and `convex/schema.ts` fails at import time if the composed tables and the inventory drift apart.
...
4. **Encoded arrays are mutable by declaration** in `ExtensionValue`/`ExtensionFieldShape` so the wire form matches Convex's mutable-array value model; keep that pattern for new array-bearing semantic values.
5. **Timestamps**: table columns are epoch-ms floats (trusted system time); timestamps inside semantic payloads remain encoded ISO strings. If A3 prefers a different storage rule, it amends `shared.ts` in sequence.
6. `parseTableId` only checks string-ness; Convex id well-formedness and existence are runtime concerns (`ctx.db.normalizeId`) proven in A3/B-lanes.
...
- `fragments.test.ts` — composed table inventory (50), pinned-validator presence, indexes for the architecture's named reads, extension version immutability, source immutability, independent parent/checklist state.
