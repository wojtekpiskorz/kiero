...
- Each implementation assignment owns one issue, one issue-specific worktree branch and one PR. A [coordinator](coordinator-start-prompt.md) may run the whole map as one goal, delegating implementation to subagents while retaining claims, Git lifecycle, integration and closure. Independent ready issues may run simultaneously. The one-issue rule applies to each worker assignment and does not end the coordinator after one issue. This supersedes any historical global single-ticket convention for this execution map.
