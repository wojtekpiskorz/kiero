...
| Containers EU jurisdiction fields | DISCHARGED (pin): wrangler `4.130.0` is pinned as the root devDependency by A3 (PR #73). Remaining action for D6/I5: set `containers[].constraints.jurisdiction = "eu"` (+ optionally `constraints.regions: ["EEUR","WEUR"]`) in `apps/*-worker/wrangler.jsonc` when those lanes deploy. THIS ROW REMAINS THE SINGLE OWNERSHIP SOURCE for the fields | pin DONE by A3; fields D6/I5 |
| Wire `infra/environments/shape-check.mjs` into CI | DISCHARGED by A3 (PR #73): `npm run verify:environments` script + `checks.yml` step now run the shape check in CI | done (A3) |
