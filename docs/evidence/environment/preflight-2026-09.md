...
node (v25). The preflight records whatever its own shell resolves; A1's pinned
toolchain supersedes both for builds.
...
Full sanitized output of the full-mode run:
...
[PENDING] cloudflare-containers-eu-jurisdiction
    ...
    ACTION: Pin wrangler >= 4.130.0 in the workspace (A1 bootstrap) before relying on container jurisdiction fields.
[VERIFIED] convex-cli
...
| Containers EU jurisdiction | PENDING | wrangler.jsonc `containers[].constraints.jurisdiction = "eu"` exists in wrangler 4.130.0's config schema but NOT in installed 4.27.0; upgrade pinned by A1 |
...
| Containers EU jurisdiction fields | Pin wrangler `>= 4.130.0` (verified schema support) in A1's workspace, then set `containers[].constraints.jurisdiction = "eu"` (+ optionally `constraints.regions: ["EEUR","WEUR"]`) in `apps/*-worker/wrangler.jsonc` | A1 pin, D6/I5 fields |
...
- `node infra/environments/shape-check.mjs` — passed: convex.json strict JSON
  ...
  and commit no `account_id`; secret-leak heuristic clean over owned paths.
- `wrangler deploy --dry-run --config apps/gateway/wrangler.jsonc` (4.27.0) —
  ...
  `src/index.ts` (A1 creates it).
- Same dry-run with `--env staging` for the three container workers parsed the
  configuration and reported only the expected missing `./Dockerfile` /
  `src/index.ts` (created by D5/D6/I5 with A1). No account or secret material
  was needed for any of these.
...
base a328471) and a scratch directory `/tmp/convex-probe` that was used only
